...
int main()
{
	...
	while(t--)
	{
		...
		vector<string> v;
		string s;
		s.pb(a[0]);s.pb(a[1]);s.pb(a[2]);
		v.pb(s);
		s.clear();
		s.pb(a[0]);s.pb(a[1]);s.pb(b[2]);
		v.pb(s);
		s.clear();

		s.pb(a[0]);s.pb(b[1]);s.pb(a[2]);
		v.pb(s);
		s.clear();
		s.pb(a[0]);s.pb(b[1]);s.pb(b[2]);
		v.pb(s);
		s.clear();

		s.pb(b[0]);s.pb(a[1]);s.pb(a[2]);
		v.pb(s);
		s.clear();
		s.pb(b[0]);s.pb(a[1]);s.pb(b[2]);
		v.pb(s);
		s.clear();

		s.pb(b[0]);s.pb(b[1]);s.pb(a[2]);
		v.pb(s);
		s.clear();
		s.pb(b[0]);s.pb(b[1]);s.pb(b[2]);
		v.pb(s);
		s.clear();
		bool say=0;
		for(int i=0;i<(int)v.size();i++)
		{
			if(v[i]=="bob"||v[i]=="obb"||v[i]=="bbo")
			{
				say=1;
				break;
			}
